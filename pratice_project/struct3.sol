...
contract StructTemi {
    ...
    */
    ...
    function addStudentStructSingText(string memory _greetings,InputStudent memory _inputStudent) public view returns(uint) {
        // in the ide to enter value makes use of [{"score": 5}]
        string memory greetings = _greetings;
        uint score =_inputStudent.score;
        return score;
    }
    ...
    /*  to call it on web through the use of metamask & etherjs npm
        ...
        const text = "Hello";            
         const update = await structTokenContract.addStudentStructSingText(text, val);
     */
    function addStudentStructArrText(string memory _greetings,InputStudent[] memory _inputStudent) public view returns(uint) {
        // in the ide to enter value makes use of [{"score": 5}]
        string memory greetings = _greetings;
        uint score =_inputStudent[0].score;
        return score;
